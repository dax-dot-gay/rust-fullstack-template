        ...
        if let Some(config) = request.rocket().state::<Config>() {
            if let Some(client) = request.rocket().state::<Client>() {
                if let Ok(sessions) = Docs::<AuthSession>::new(client.clone()).await {
                    if let Some(cookie) = request.cookies().get_private("fullstack:token") {
                    ...
                    request.cookies().add_private(Cookie::build(("fullstack:token", new_id._id.clone())).expires(OffsetDateTime::from_unix_timestamp(new_id.get_expiry(config.clone()).timestamp()).expect("Expiration out of range")));
                }
            }
        }
    }
